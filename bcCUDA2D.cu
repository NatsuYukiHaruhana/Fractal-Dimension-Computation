...
__global__ void BCKernel2D(unsigned char* M, const long long int m, const unsigned long long int bits_m, const unsigned long long int sm,
						   const unsigned long long bits_s, const unsigned long long bits_TPB, float* n)
{
...
cudaError_t CudaBC2D(const unsigned char* M, const long long int m, const unsigned char bits_m, const unsigned int TPB, const int nn, float* n)
{
	...
	float* device_n = 0; // array n in GPU device
	cudaStatus = cudaMalloc((void**)&device_M, m * m * sizeof(unsigned char));
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMalloc failed!");
		return cudaStatus;
	}
    cudaStatus = cudaMalloc((void**)&device_n, nn * sizeof(float));
    if (cudaStatus != cudaSuccess) {
        fprintf(stderr, "cudaMalloc failed!");
		cudaFree(device_M);
		return cudaStatus;
	...
	cudaStatus = cudaMemset(device_n, 0, nn * sizeof(float));
	if (cudaStatus != cudaSuccess) {
	...
	cudaStatus = cudaMemcpy(n, device_n, nn * sizeof(float), cudaMemcpyDeviceToHost);
